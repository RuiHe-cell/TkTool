...
class DataGeneratorCore:
    ...
    def generate_preview_data(self, configs: List[Dict[str, Any]], count: int = 3, no_duplicate: bool = False) -> List[str]:
        """生成预览数据
    ...
    def _generate_single_value(self, config: Dict[str, Any], variable_values: Dict[str, Any] = None) -> Union[int, float, str]:
        """生成单个值
        ...
        if source_type == "数据范围":
            ...
            return self._generate_from_charset(config)
        elif source_type == "来自代码":
            return self._generate_from_code(config)
        else:
            raise ValueError(f"未知的数据源类型: {source_type}")
    
    def _generate_from_range(self, config: Dict[str, Any], variable_values: Dict[str, Any] = None) -> Union[int, float, str]:
        """从数据范围生成值"""
        if variable_values is None:
            variable_values = {}
        ...
        if data_type == "整数":
            ...
            # 对于字符串，范围表示ASCII码范围
            length = self._parse_string_length(config.get('string_length', '10'))
            result = []
    ...
    def _generate_from_charset(self, config: Dict[str, Any]) -> str:
        """从字符集合生成值"""
    ...
    def _parse_string_length(self, length_str: str) -> int:
        """解析字符串长度配置
        
        Args:
            length_str: 长度配置字符串，支持固定长度（如"10"）或随机长度（如"1,10"）
